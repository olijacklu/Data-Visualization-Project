...
- Provides out-of school data by country & year
- Out-of school child = child that doesn't attend a school of any form, regardless of the reason

```{r}
summary(read_csv("data/education_data.csv", show_col_types = FALSE))
```
...
education |>
  ...
  ggplot(aes(x = gdp_pc, y = oos_rate, colour = continent)) +
  geom_point() +
...
education |>
  ...
  ggplot(aes(x = unemployment_p, y = oos_rate, colour = continent)) +
  geom_point() +
  scale_size(range = c(2, 12)) +
